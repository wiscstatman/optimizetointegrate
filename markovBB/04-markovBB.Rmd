...
The probabilistic machinery through which the Dirichlet Process (DP) calculations are developed rests in part on exchangeability assumptions, reinforcement learning, predictive modeling, and urn sampling.   We refer the reader to **cite** for a lucid development, and we note for our purposes with Markov processes that a simple urn-sampling metaphor is helpful.  Namely, we suppose that prior to data being in hand, there is an urn containing a measure $\alpha_0 F_0$; roughly, this measure is supposed to represent the analyst's uncertainty about any data point $Z_i$, prior to observation.  After seeing $Z_1$, the analyst's mind updates -- specifically the measure goes from $\alpha_i F_0$ to $\alpha_0 F_0 + \delta_{Z_1}$, by adding a point mass at $Z_1$.  Subsequent measurements continue to add mass, until the urn contains measure $\alpha_0 F_0 + n \mathbb{F}_n$ by the end of data collection.  This measure, as it turns out, is the centering measure of the posterior DP.  Random weighting emerges, a posteriori, by sending $\alpha_0 \longrightarrow 0$ to eliminate $F_0$. (Note that urn sampling is like drawing a value from normalized probability distribution residing within the urn; the measure-adding reinforcement conveys how DP analysts learn from data.) (maybe comment on how weights can be generated from the fixed urn at the end of data
collection)
...
view.  In the absence of any regularizing assumptions about the kernel $F(z,A)$, we might be obliged to update the urns as follows.   Say with $Z_1$ fixed, let's update urn $U_{Z_1}$ from its original state by adding point mass $\delta_{Z_2}$; likewise at any transition from $Z_i$ to $Z_{i+1}$, update the urn $U_{Z_i}$ from its prior state by adding point mass at $Z_{i+1}$.  For continuous recordings, this approach doesn't get us too far, even after all data are in hand, since we may expect to see single point masses added to $n-1$ of the continuum of urns.  And then any limits we take on $\alpha_0$ will leave us with very little empirical variation (probably just one single trajectory is realizable).

**Regularizing assumption:**  Suppose instead that nearby urns (i.e. urns at nearby state
space values $z$ and $z'$) tend to be similar, and, further, that all urns share the same support.
...
point $Z_{i+1}$ emmitted after point $Z_i$:
\begin{eqnarray*}
...
**Conjecture:** Letting $n$ get large and for sufficiently large $\tau$, the normalized urns will converge to the generative kernel $F$.

```{r setupsimulate}

# See first calcs in 03-*R

# Weight associated with distance affects the next point that
