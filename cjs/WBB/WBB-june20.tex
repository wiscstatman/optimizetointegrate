...
%% \footnotetext{We thank the associate editor and referees for their detailed, helpful comments.}}
\author{Michael A. Newton\authorref{1}}
\author{Nicholas G. Polson\authorref{2}}
\author{Jianeng Xu\authorref{2}}
\affiliation[1]{Departments of Statistics and of Biostatistics and Medical Informatics, University of Wisconsin-Madison, Madison, WI, U.S.A.. To whom correspondence may be addressed: newton@stat.wisc.edu}
\affiliation[2]{Booth School of Business, University of Chicago, Chicago, IL, U.S.A.}
%\author{Blinded-A}
%\author{Blinded-B}
%\author{Blinded-C}
...
deep learning\sep Markov chain Monte Carlo\sep regularization\sep trend filtering\sep weighted bootstrap.
% MSC 2010 subject classification codes
\KWDtitle{MSC 2010}Primary 62F40\sep secondary 62F15}
\begin{abstract}
\abstractsection{}
...
Weighted Bayesian Bootstrap (WBB) is a simulation-based algorithm for assessing uncertainty in Machine Learning and Statistics. Uncertainty quantification (UQ) is an active area of research, particularly in high-dimensional inference problems (Wang \& Swiler, 2018). 
...
\begin{eqnarray}
\label{eqn:canonicalform}
 \underset{\theta \in \R^d}{\text{minimize}} \quad
  \mathcal{L}(\theta) \defeq l(y| \theta) + \lambda\phi(\theta) 
\end{eqnarray}
...
The maximum likelihood estimator (MLE) is  $ \hat{\theta} \defeq {\rm arg max}_\theta\; p(y |\theta) $, though
of course this usually differs from the solution to Equation~(\ref{eqn:canonicalform}): $\theta^* \defeq \argmin \left\{ l(y|\theta) + \lambda \phi(\theta)
...
for example, in regression with a Gaussian regression model
...
a solution to Equation~(\ref{eqn:canonicalform}).
%\begin{eqnarray}
%\theta^* & =& \underset{\theta \in \Theta}{\argmin}\,  \{ l(y|\theta) + \lambda\phi(\theta) \}.  \label{posteriormode}
%\end{eqnarray}
See Gribonval \& Machart (2013) for a nuanced view of the connection between 
Equation~(\ref{eqn:canonicalform}) and Equation~(\ref{eqn:bayes}) in
...
Equation~(\ref{eqn:subdiffproxgrad}) usefully guides algorithms that aim to solve $\theta^*$.   
...
\noindent We now define the weighted Bayesian bootstrap (WBB).  Recalling the original objective function Equation~(\ref{eqn:canonicalform}),
...
induced by ${\bf w}$ with the data fixed --  approximates the Bayesian posterior Equation~(\ref{eqn:bayes}). For any measurable set $\mathcal{B}$ in the parameter space,
\begin{eqnarray}
\label{eqn:conjecture}
{\rm Pr}\left( \theta^*_{\bf w} \in \mathcal{B} | y \right) \approx \int_\mathcal{B} p(\theta|y) \, d\theta.
\end{eqnarray}
Section 2.5 provides an asymptotic argument in support of  Equation~(\ref{eqn:conjecture}), and we investigate
...
When optimization on the original problem Equation~(\ref{eqn:canonicalform}) is fast and scalable, so too is the WBB. Weights ${\bf w}$ and the corresponding  $\theta^*_{\bf w}$ are independent across $t$, making it possible to speed up the algorithm via parallel computing.
...
Next we consider the approximation Equation~(\ref{eqn:conjecture}) from an asymptotic perspective.


\subsection{WBB Properties}

Conditions under which the target posterior distribution Equation~(\ref{eqn:bayes}) is approximately Gaussian are well established (Kleijn \&  van der Vaart, 2012).  For example, when data form a random sample from fixed distribution $p(y_i|\theta_0)$
...
out the left hand side of Equation~(\ref{eq:wbbTaylor}) and leads to:
\begin{eqnarray}
\label{eq:asymp}
...
\end{eqnarray}
where $\tilde R_n$ is another error term.  Following Newton \& Raftery (1994), we recognize that the ${\bf w}$-induced variation in Equation~(\ref{eq:asymp}), conditional upon 
...
negligible.   Thus, compared to the target posterior variation Equation~(\ref{eq:postG}), we 
have WBB variation:
\begin{eqnarray*}
...
We note that rescaling in Equation~(\ref{eqn:rand}) has no effect on
...
\begin{figure}[!ht]
\centering 
\includegraphics[scale=0.55]{1-simple.pdf} 
...
permit a clear description of the prior that is in force for this sampled distribution.
...
\includegraphics[scale=0.65]{1-filtering.pdf} 
\caption{Cubic trend filtering: the red line is $\hat\beta_i$ for $i=1,2,...500$; the blue line is $\hat\beta_i \pm 2 \times se$ where the standard errors are computed by WBB; $\lambda = 1000$.}
\label{filter}
\end{figure}
...
\begin{figure}[!ht]
	...
	\includegraphics[scale=0.55]{acc}
	\caption{Posterior distribution of the classification accuracy by WBB. $n=500, \lambda=10^{-4}$.}
	\label{fig:acc}
\end{figure}
...
Chollet, F.  (2015). Keras {\it https://keras.io}.
...
Lange, K. (2016). {\em MM Optimization Algorithms}. Society for Industrial and Applied Mathematics,
	Philadelphia, PA.
...
LeCun, Y. \& Cortes, C. (2010). MNIST handwritten digit database.  \newline
 Retrieved from {\it http://yann.lecun.com/exdb/mnist/}

\bibitem[Liang \& Hu, 2015]{liang2015recurrent}
Liang, M. \& Hu, X. (2015). Recurrent convolutional neural network for object recognition. {\it In Proceedings of the IEEE conference on computer vision and pattern recognition},  3367-3375.
...
Mitchell, A. F. (1994). A note on posterior moments for a normal mean with double- exponential prior. {\it Journal of the Royal Statistical Society. Series B (Methodological)}, 56(4), 605--610.
