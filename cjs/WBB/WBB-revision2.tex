...
{\bf **something about BLASSO being on a different prior than the WBB penalty**}

Before we look into the simulation results, it's worth mentioning that WBB algorithm samples from a posterior with slightly different prior than the prior used in the penalty. In the simple example shown in Figure (\ref{mcmc}), we fix prior as $p(\beta) \propto \exp\{-\lambda \Vert \beta\Vert_1\}$ and set $\lambda = 10, n=50, p=2, \sigma_\epsilon^2=1, \Sigma = [[1, 0.3]', [0.3,1]']$ and $\beta_1 = 0, \beta_2 = 2$. Both posteriors from WBB and MCMC are presented. For comparison, MCMC posteriors with $L^0$ priors, $p(\beta) \propto \exp\{-\lambda \Vert \beta\Vert_0\}$ and $p(\beta) \propto \exp\{-5\lambda \Vert \beta\Vert_0\}$, are also shown in the same figure. The WBB posterior has a spike at $\beta_1 = 0$, indicating a positive probability there, which is never the case under LASSO or $L^0$ prior.


\begin{figure}[!ht]
\centering 
\includegraphics[scale=0.55]{mcmc} 
\caption{WBB posterior under LASSO prior, compared with MCMC posteriors under  LASSO prior and $L^0$ prior.}
\label{mcmc}
\end{figure}
...
The complete results for estimation error,  out-of-sample prediction error, credible interval coverage and model-selection discrepancy are displayed in Table (1), (2), (3) and (4), respectively. In almost all cases when $p\geq 60$, WBB has lower estimation error than BLASSO. 
...
There the WBB mean model-selection discrepancy is substantially 
smaller than that of BLASSO. However, its performance gets worse when $\beta$ is dense and uniform.  Though limited in scope, this simulation reveals some distinctions
