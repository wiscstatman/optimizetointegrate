...
\begin{algorithm} [H]
        ...
        \textbf{Input:}  \begin{list}{}
                         ...
                         \item model structure: ${\mathcal M}=(l,\lambda,\phi)$ 
                         \item number of samples: $N_{\rm sim}$ 
	                \end{list}
        \textbf{Output:} $N_{\rm sim}$ parameter samples $\{ \theta^{*,k} \}$
        \begin{algorithmic}
            \State  Function ${\bf WBB}({\mathcal D},{\mathcal M},N_{\rm sim})$:

            \ForAll  {$k=1$ to $N_{\rm sim}$}
		...
		\State Compute: $\theta^{*,k} \gets \argmin {\mathcal L}_{\bf w}(\theta)$
