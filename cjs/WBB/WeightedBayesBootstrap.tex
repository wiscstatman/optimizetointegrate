...
$$
y = A\beta + \epsilon, \; \epsilon \sim N(0, \sigma_\epsilon^2 I)
$$
where $A$ is $n\times p$ and $y$ is $n\times 1$. The design matrix, $A=[A_1, A_2, ..., A_p]$, is generated from the distribution $A \sim N(0, \Sigma)$. The covariance between $A_i$ and $A_j$ is set to be $\Sigma_{i,j} = 0.1*0.8^{|i-j|}$. Here we also assume i.i.d Gaussian noise where $\sigma_\epsilon = \sqrt{\Vert A\beta \Vert^2_2/(2n)}$. In this setting, the signal-noise ratio (SNR) is 2. In terms of the coefficient vector $\beta = [\beta_1, \beta_2, ..., \beta_p]'$, we consider both sparse and dense settings: 
\begin{enumerate}
