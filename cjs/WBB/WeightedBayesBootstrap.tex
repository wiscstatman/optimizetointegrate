...
problems (e.g., Wang, 2018). 
Whilst there are computationally fast and scalable algorithms for training models in a wide variety of contexts, uncertainty assessments are still required, as are methods to compute these assessments.  Bayesian analysis
...
The penalty function effects a favorable bias-variance 
...
see Wright (2015), or  Hastie, Tibshirani, and Wainwright (2015, chap. 5) for a statistical perspective.
...
which opens the door to powerful MM algorithms and related schemes; 
 see Lange (2016, chap. 5) and Polson \& Scott (2015a, 2015b).  Beyond convexity, the guarantees are weaker (e.g., local
...
(\verb+http://cran.r-project.org+). For machine learning, the \verb+TensorFlow+ system has greatly simplified 
...
\begin{eqnarray}
\label{eqn:rand}
\mathcal{L}_{\bf w}( \theta ) = \left\{ \sum_{i=1}^n w_i l_i( y_i | \theta ) \right\} + w_{0}\lambda \phi(\theta)
\end{eqnarray}
%$$
%{\bf w} = (w_1, ..., w_n, w_p),\,
...
Equivalently, 
$w_i = \log(1/u_i)$ where $u_i$'s are i.i.d. Uniform$(0,1)$. 
...
induced by ${\bf w}$ with the data fixed --  approximates the Bayesian posterior~(\ref{eqn:bayes}): for sets $B$ 
in the parameter space,
\begin{eqnarray}
\label{eqn:conjecture}
{\rm Pr}\left( \theta^*_{\bf w} \in B | y \right) \approx \int_B p(\theta|y) \, d\theta.
\end{eqnarray}
...
\begin{algorithm} [H]
		...
		\State Construct: $w_i \gets \log(1/u_i), \quad \forall i, \quad {\bf w}=(w_0, w_1, \cdots, w_n)$ 
...
When optimization on the original problem~(\ref{eqn:canonicalform}) is fast and scalable, so too is the WBB. Next we consider the approximation~(\ref{eqn:conjecture}) from an asymptotic perspective.
...
See Appendix and Polson \& Sokolov (2017)  for further discussion. 
 To choose the amount of regularization $\lambda$, we can use the marginal likelihood $m_\lambda (y)$, estimated by bridge sampling (Gelman \& Meng, 1998) or simply using predictive cross-validation.
**

\subsection{WBB Properties}

Conditions under which the target posterior distribution~(\ref{eqn:bayes}) is approximately Gaussian are well established (e.g., 
Johnson, 1970; Kleijn \& van der Vaart, 2012).  For example, when data forms a random sample from fixed distribution $p(y_i|\theta_0)$
that resides in a sufficiently regular model, and when the prior is smooth and positive around $\theta_0 \in \R^d$,  we have 
\begin{eqnarray}
\label{eq:postG}
\left. \theta \right| y  \sim_{\rm approx} N_d \left( \theta_n^*, J_n^{-1}(\theta^*_n)\right),
\end{eqnarray}
where, including sample size $n$ as an explicit subscript, we have posterior mode $\theta_n^* = \argmin {\mathcal L}_n( \theta)$, and
where $J_n(\theta^*_n)=nj(\theta_n^*)$ is the Fisher information matrix evaluted at $\theta_n^*$.  
Here $j(\theta)$ is the information per sample, and $y=(y_1, \cdots, y_n)$ denotes data.
Centering on the posterior mode, rather than the MLE, improves accuracy in many cases (?citation?).   
...
\begin{eqnarray}
\label{eq:postW}
\left. \theta^*_{{\bf w},n} \right| y  \sim_{\rm approx} N_d \left( \theta_n^*, J_n^{-1}(\theta^*_n)\right).
\end{eqnarray}
In a relatively narrow asymptotic sense, therefore, the WBB procedure is approximating the
target posterior distribution, in that both are approximately Gaussian with the same mean
and covariance.   Details of the asymptotic analysis follow the WLB case presented in 
Newton (1991) and Newton and Raftery (1994), 
and differ only slightly in our use of posterior mode $\theta^*_n$ in place
of maximum likelihood estimator $\hat \theta_n$, and also in our incorporation of weight
$w_0$ on the penalty term of the objective function.   At this first-order 
level of asymptotic analysis, neither of these features affects the limiting distribution
of $\theta^*_{{\bf w},n}$.



**Nick,   you wrote *$J_n = nj$ as `observed information`, but if it's the curvature of the observed log likelihood then I guess it's not $n j$.
 Do you mean Fisher information?**



* asymptotic analysis limited...e.g. requires smooth prior and fixed dimension...alternatives*

* throughout we use exponential weights...scaling not NB...in fact maybe expo not important
 (cite Barbe and Bertaile), except through connection to Bayes and the Dirichlet...one
 curious fact is that the expo gives uniform on the simplex...
...
Given the i.i.d. exponential weights $w_0$ and $w_1$, the weighted posterior mode $\theta^*_{\bf w}$ is 
$$
...
\end{equation}
The regularization factor $\lambda$ is chosen by cross-validation with unweighted likelihood. The weighted Bayesian Bootstrap is also performed with fixed prior, namely, $w_{0}$ is set to be 1 for all bootstrap samples. \cite{polson2014Bayesian} analyze the same dataset using the Bayesian Bridge estimator and suggest MCMC sampling from the posterior. 
...
\end{eqnarray*}
where $$W = {\rm diag}\left(\sqrt{w_1}/\sqrt{w_{0}},\cdots,  \sqrt{w_p}/\sqrt{w_{0}}\right)$$ and $$\tilde{y}_{\bf w} = Wy, \, \tilde{\beta}_{\bf w} = W\beta, \,   \tilde{D}^{(k)}_{\bf w}= D^{(k)}W^{-1}.$$
...
Deep learning is a form of machine learning that uses hierarchical abstract layers of latent variables to perform pattern matching and prediction.  Polson \& Sokolov (2017)
 take a Bayesian probabilistic perspective and provide a number of insights into more efficient algorithms for optimization
