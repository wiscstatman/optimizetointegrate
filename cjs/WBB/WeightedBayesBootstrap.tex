...
Advances in the theory of optimization provide efficient algorithms
...
induced by ${\bf w}$ and fixing the data --  approximates the Bayesian posterior~(\ref{eqn:bayes}): for sets $B$ 
in the parameter space,
\begin{eqnarray}
\label{eqn:conjecture}
{\rm Pr}\left( \theta^*_{\bf w} \in B | y \right) \approx \int_B p(\theta|y) \, d\theta.
\end{eqnarray}
Section 2.5 provides a limited asymptotic argument in support of~(\ref{eqn:conjecture}), and we investigate
the approximation numerically in a few examples in Section 3.  We note that rescaling in~(\ref{eqn:rand}) has no effect on 
...
\begin{algorithmic}[1]
\Procedure{WBB}{}
\State $(u_0, u_1, \cdots, u_n) \gets$ Uniform$(0,1)$ random variables
\State $w_i \gets \log(1/u_i), \quad \forall i, \quad {\bf w}=(w_0, w_1, \cdots, w_n)$ 
\State $\theta^*_{\bf w} \gets \argmin {\mathcal L}_{\bf w}(\theta)$
\EndProcedure
\end{algorithmic}
\end{algorithm}
