...
The penalty function $\phi(\theta) = \sum_{j=1}^p \phi(\theta_j)$ is assumed to be separable and effects a favorable bias-variance 
...
\begin{eqnarray}
\label{eqn:rand}
\mathcal{L}_{\bf w}( \theta ) = \left\{ \sum_{i=1}^n w_i^l l_i( y_i | \theta ) \right\} + \lambda \sum_{j=1}^p w_j^\phi \phi(\theta_j)
\end{eqnarray}
%$$
%{\bf w} = (w_1, ..., w_n, w_p),\,
%p_{\bf w}(\theta | y) \propto \prod_{i=1}^n p(y_i|\theta)^{w_i} p(\theta)^{w_p}
%$$
where entries of  ${\bf w}=(w_1^l, \cdots, w_n^l, w_1^\phi, ..., w_p^\phi)$ are independent and identically distributed (i.i.d.) 
standard exponentially distributed random weights, generated by the analyst and independently from the data $y$. 
Equivalently, 
$w_k = \log(1/u_k)$ where $u_k$'s are i.i.d. Uniform$(0,1)$. 
...
induced by ${\bf w}$ with the data fixed --  approximates the Bayesian posterior~(\ref{eqn:bayes}): for any measure set $\mathcal{B}$ 
...
\begin{algorithm} [H]
        ...
        \begin{algorithmic}
            ...
            \ForAll  {$t=1$ to $T$}
		\State Realize: $(u_1, \cdots, u_n, u_{n+1}, \cdots, u_{n+p}) \sim_{i.i.d.}\, $Uniform$(0,1)$ 
		\State Construct: $w_i^l \gets \log(1/u_i), w_j^\phi \gets \log(1/u_{n+j}), \quad 1 \leq i \leq n, \;1 \leq j \leq p$. %${\bf w}=(w_1, \cdots, w_n, w_{n+1}, \cdots, w_{n+p})$ 
...
$(w_{n+1}, ..., w_{n+p})$ on the penalty term of the objective function.   At this level of first-order 
...
Given the i.i.d. exponential weights $w^l$ and $w^\phi$, the weighted posterior mode $\theta^*_{\bf w}$ is given by 
$$
\theta^*_{\bf w} = \underset{\theta \in \Theta}{\argmin}\, \left\{\frac{w^l}{2} (y-\theta)^2 + \lambda w^\phi |\theta|\right\}.
$$
This is sufficiently simple for an exact WBB solution in terms of the soft thresholding proximal operator:
$$
\theta^*_{\bf w} = 
\begin{cases}
y - \lambda w^\phi /w^l &\mbox{if $y > \lambda w^\phi /w^l$},\\
y + \lambda w^\phi /w^l &\mbox{if $y < -\lambda w^\phi /w^l$},\\
0 &\mbox{if $|y| \leq \lambda w^\phi/w^l$}.\\
\end{cases}
$$
