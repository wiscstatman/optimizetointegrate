...
The penalty function or regularization term, $\lambda\phi(\theta) $,   encodes soft or hard constraints on the solution, and,
...
Extensive research and development in optimization give us efficient algorithms and software tools
to compute solutions, $\theta^*$, to~(\ref{eqn:canonicalform}), for various loss and penalty functions. 
...
\begin{eqnarray*}
\left\{ \sum_{i=1}^n w_i l_i( y_i | \theta ) \right\} + w_{0}\lambda \phi(\theta)
\end{eqnarray*}
%$$
%{\bf w} = (w_1, ..., w_n, w_p),\,
%p_{\bf w}(\theta | y) \propto \prod_{i=1}^n p(y_i|\theta)^{w_i} p(\theta)^{w_p}
%$$
where the weights $w_0, w_1, \cdots, w_n \sim_{i.i.d.} {\rm Exp}(1)$ are exponentially distributed random weights. 
It's equivalent to draw
