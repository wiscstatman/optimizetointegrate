...
high dimensional posterior modes are now readily computable, thanks to systems such as TensorFlow (Abadi et al., 2015) and Keras (Chollet et al., 2015)
that deploy stochastic gradient descent (SGD) and convex optimization methods (Boyd \& Vandenberghe, 2004) for large-scale
...
The maximimum likelihood estimator (MLE) is  $ \hat{\theta}_n \defeq {\rm arg max}_\theta\; p(y |\theta) $, though
...
\begin{eqnarray}
\theta^*_n & =& \underset{\theta \in \Theta}{\argmin}\,  \{ l(y|\theta) + \lambda\phi(\theta) \}.  \label{posteriormode}
\end{eqnarray}

\subsection{Optimization}

Let $\partial$ be the subdifferential operator.  
Then a necessary and sufficient condition for $\theta^*$ to minimize $ l(y|\theta) + \lambda\phi(\theta)$  is 
\begin{equation}
