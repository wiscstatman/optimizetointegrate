...
Equivalently, 
$w_i = \log(1/u_i)$ where $u_i$'s are i.i.d. Uniform$(0,1)$. When $p>n$ and $\phi(\theta)$ is separable as $\phi(\theta) = \sum_{j=1}^p \phi_j(\theta_j)$, we recommend individual weight on each prior regularization term $\phi_j(\theta_j)$, namely $\lambda \sum_{j=1}^p w_j^\phi \phi_j(\theta_j)$, to help with sparsity and to reduce the effect of occasionally large random weight $w_0$.
