...
The penalty function, or regularization term, $\lambda\phi(\theta) $,   may encode soft or hard constraints, and is controlled by 
a hyper-parameter, $\lambda>0$, whose values index an entire path of solutions. 
From the perspective of estimation, the penalty function effects a favorable bias-variance tradeoff.   
...
(\verb+http://cran.r-project.org+). In machine learning, the \verb+TensorFlow+ system has greatly simplified 
