...
We provide regularity conditions which apply to a range of machine learning and statistical models. We illustrate our methodology in regularized regression, trend filtering and deep learning and conclude with directions for future research.
...
We illustrate our methodology with a number of scenarios to assess 
...
For each setting of $\beta$, we consider $p = 40, 60, 80, 100, 120$. To investigate the performance of our methodology under different sample size, we also choose two settings of $n$:
\begin{enumerate}
...
We view the WBB samples as approximate posterior draws, though the algorithm structure does not
...
penalty used in repeated optimization.   Figure (\ref{mcmc}) displays WBB samples (kernel density
...
two $L^0$ penalties,  $p(\beta) \propto \exp\{-\lambda \Vert \beta\Vert_0\}$ and $p(\beta) \propto \exp\{-5\lambda \Vert \beta\Vert_0\}$. 
...
\caption{WBB samples under LASSO penalty, compared with MCMC posteriors under  LASSO prior and $L^0$ prior.}
\label{mcmc}
\end{figure}
